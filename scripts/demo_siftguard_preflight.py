...
def _mcp_tools(repo: Path) -> set[str]:
    ...
    completed = _run(
        [str(repo / ".venv" / "bin" / "python"), "-m", "siftguard.integrations.mcp_server"],
        cwd=repo,
        input_text=request,
    )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Check that the SIFTGuard demo uses the bounded MCP path."
    )
