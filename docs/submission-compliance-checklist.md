...
| Self-correction demonstration | Demo and logs show OpenClaw discovering the real ROCBA theft/exfiltration evidence gap and revising posture without treating model output as evidence. | `docs/demo.md`, `docs/demo-script.md`, `docs/openclaw-mcp-workflow.md`, generated `self_correction_events.json`. | `[ ]` |
...
| Third-party tool/license notes | Third-party SIFT tools, Python dependencies, and runtime assumptions are documented; no incompatible third-party assets are committed. | `pyproject.toml`, `docs/parser-tooling-matrix.md`, `docs/openclaw-mcp-workflow.md`. | `[ ]` |
